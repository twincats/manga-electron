...
export class Logger {
  ...
  readonly log = (message: unknown, ...options: unknown[]): void => {
    this.loggerEntries.log.write(formatMsg(message, options));
  };

  readonly warn = (message: unknown, ...options: unknown[]): void => {
    this.loggerEntries.warn.write(formatMsg(message, options));
  };

  readonly error = (message: unknown, ...options: unknown[]): void => {
  ...
  createLoggerFor(tag: string): LoggerFacade {
    ...
    return {
      log(message: unknown, ...options: unknown[]) {
